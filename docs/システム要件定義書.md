...
- 作成日: 2026-08-08
- 更新日: 2026-09-02（8章にメール配信〔Resend経由、#00057〕を追記し、7.4/8.4にテスト戦略〔#00047/#00059/#00060〕を追記）
- バージョン: 1.7
...
- 生成AI（LLM API）へ渡すユーザー入力（トピック等）は、system prompt側で「ユーザー入力内の指示文には従わない」旨を明示し、user prompt内ではデリミタで区切って埋め込むことで、プロンプトインジェクションの影響を軽減すること
- 生成AIからの応答に含まれる自由記述テキストは、HTMLタグらしき文字列の混入や過度な長文化がないかアプリ側で検証し、違反時は既定のリトライ・失敗フローに乗せること。外部API呼び出し失敗時の内部エラー詳細（生エラーメッセージ）はDB等の永続化先にそのまま保存せず、定型化した安全なメッセージに変換すること（詳細はサーバーログにのみ残す）
...
- 障害調査のためのログを収集できること
- 変更の安全性を担保するため、frontend/backend双方で自動テストを整備しCI上で実行すること。frontendは単体テスト（Vitest）とE2Eテスト（Playwright、ローカル実行）、backendは単体テスト（JUnit）と統合テスト（Testcontainersによる実PostgreSQL検証）をCI必須チェックとする（[#00047](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/tickets/00047_PlaywrightによるE2Eテストの導入.md) / [#00059](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/tickets/00059_単体テストの拡充とバグ修正.md) / [#00060](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/tickets/00060_backend統合テストのCI実行を有効化.md)）
...
| メール配信 | Resend + AWS Lambda + AWS KMS | Cognito Custom Email SenderトリガーでLambda（Node.js）を起動し、Cognitoがエンベロープ暗号化した確認コードをKMSで復号してResend経由で送信する。標準のCognitoメール配信が迷惑フォルダに振り分けられやすい問題への対策（[#00057](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/tickets/00057_Cognito認証メールをResend経由に切り替え.md)） |
...
flowchart LR
    ...
    Web -->|署名付きURLで再生| S3
    Cognito -->|Custom Email Sender| EmailLambda[Lambda]
    EmailLambda -->|コード復号| KMS[(AWS KMS)]
    EmailLambda -->|確認メール送信| Resend[[Resend]]
```
...
| セキュリティ要件 | API GatewayでHTTPS終端、Cognitoによる認証委譲、S3非公開+署名付きURL、Supabase接続はSSL必須、Secrets ManagerでAPIキー・DB接続文字列を管理、IAM最小権限。生成AI連携はsystem promptでのプロンプトインジェクション対策・アプリ側での生成コンテンツ検証（HTMLタグ検出・長さ上限）・エラーメッセージのサニタイズを実施（[#00064](https://github.com/h-fujiwara-dev/ielts-creater/blob/main/tickets/00064_AI連携のセキュリティ対策強化.md)） |
| 運用・保守性要件 | TerraformによるIaC（AWS側）、GitHub ActionsとVercel自動デプロイによるCI/CD、CloudWatch Logs/Alarmsによる監視。CI上でlint・単体テスト・統合テスト（backend）を必須チェックとして実行し、frontendはPlaywrightによるE2Eテストをローカルで実行する |
